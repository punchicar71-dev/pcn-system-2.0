...
2. **Session Cookies**: `pcn-dashboard.session_token` cookie (stores user ID)
3. **Client Storage**: `localStorage` for user data (temporary)
...
- **Authentication System**: Custom Auth (Migration to Better Auth in progress)
...
### Step 2: Better Auth Integration 🔄 PENDING
...
4. Update middleware with proper session validation
5. Migrate existing password hashes to bcrypt/argon2
6. Remove localStorage-based temporary auth
7. Implement proper JWT session tokens
