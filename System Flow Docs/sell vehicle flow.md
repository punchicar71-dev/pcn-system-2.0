...
- **Storage**: Combined with Last Name into `customer_name` column
- **Validation**: Required field validation
- **Component**: Text input field

#### 3. **Last Name** (Required)
- **Storage**: Combined with First Name into `customer_name` column
- **Validation**: Required field validation
- **Component**: Text input field
...
- **Storage**: `sale_price` (DECIMAL) in `pending_vehicle_sales`
...
CREATE TABLE pending_vehicle_sales (
  ...
  customer_name VARCHAR(200) NOT NULL, -- Combined first + last name
  customer_address TEXT,
  customer_nic VARCHAR(50),
  customer_mobile VARCHAR(20),
  
  -- Sale Details
  sale_price DECIMAL(12, 2) NOT NULL, -- Selling price
...
const handleSubmitSale = async () => {
  ...
  try {
    ...
    // Combine first and last name into customer_name
    const customerName = `${customerData.firstName} ${customerData.lastName}`.trim();
    ...
    const saleData: Record<string, any> = {
      ...
      customer_nic: customerData.nicNumber || null,
      customer_mobile: customerData.mobileNumber || null,
      sale_price: salePrice, // Database field is sale_price
      ...
      third_party_agent: showroomAgentName || '', // Showroom agent name
      status: 'pending',
    };
...
> **⚠️ SECURITY UPDATE (December 29, 2025)**: SMS API endpoints are now protected with authentication and rate limiting. All SMS messages are built server-side using templates - clients cannot send arbitrary message content.
...
### Security Features (Added Dec 29, 2025)

| Feature | Description |
|---------|-------------|
| **Authentication** | API requires valid session token |
| **Rate Limiting** | 10 SMS per hour per user |
| **Template-Based** | Messages built server-side only |
| **No Arbitrary Messages** | Clients send structured data, not raw messages |
| **Environment Variables** | API tokens loaded from `.env.local` only |

### SMS Message Templates

All SMS messages are defined server-side in `dashboard/src/lib/sms-service.ts`:

**Available Templates**:
- `vehicle-acceptance` - Vehicle added to showroom
- `sell-vehicle-confirmation` - Vehicle sold confirmation
...
### SMS Message Format (sell-vehicle-confirmation)
...
Contact: 0112 413 865 | 0117 275 275
...
1. **Authentication Check**: Verify user session token (401 if invalid)
2. **Rate Limit Check**: Verify user hasn't exceeded 10 SMS/hour (429 if exceeded)
3. **Fetch Seller Data**: Get original seller from `sellers` table
4. **Validate Data**: Check seller name and mobile number exist
5. **Build Message Server-Side**: Message built from template (not from client)
6. **Call API**: POST to `/api/vehicles/send-sms`
7. **Handle Response**: Log success/failure, don't block sale if SMS fails

### SMS Error Handling

- **Unauthorized**: 401 error if session invalid
- **Rate Limited**: 429 error if quota exceeded
